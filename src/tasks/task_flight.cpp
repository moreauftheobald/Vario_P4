...
/**
 * @brief Lecture capteurs avec monitoring santé
 */
void task_flight_read_sensors() {
    ...
    if (sensor_lsm6dso32_ready) {
        ...
        if (lsm6dso32.getEvent(&accel, &gyro, &temp)) {
            // Lecture OK - récupérer données brutes
            gyro_x = gyro.gyro.x;
            gyro_y = gyro.gyro.y;
            gyro_z = gyro.gyro.z;
            ...
            if (g_flight_data.sensors_health.imu_error_count == SENSOR_ERROR_THRESHOLD) {
                LOG_E(LOG_MODULE_FLIGHT, "IMU FAILED after %d consecutive errors!", 
                      SENSOR_ERROR_THRESHOLD);
                g_flight_data.sensors_health.imu_healthy = false;
            }
        }
        
        // Vérifier timeout (pas de lecture depuis 5s)
        if (millis() - g_flight_data.sensors_health.last_imu_success > SENSOR_TIMEOUT_MS) {
            if (g_flight_data.sensors_health.imu_healthy) {
                LOG_E(LOG_MODULE_FLIGHT, "IMU TIMEOUT (no data for %d ms)", 
                      millis() - g_flight_data.sensors_health.last_imu_success);
                g_flight_data.sensors_health.imu_healthy = false;
            }
        }
    }
    ...
    if (sensor_gps_ready && (counter_main % FREQ_GPS_DIVIDER == 0)) {
        ...
        if (gps.newNMEAreceived()) {
            if (gps.parse(gps.lastNMEA())) {
                ...
                gps_speed = gps.speed * 0.51444f;  // Convertir knots en m/s
                ...
                if (gps.fix) {
                    ...
                    g_flight_data.sensors_health.gps_error_count++;
                    
                    if (g_flight_data.sensors_health.gps_error_count == SENSOR_ERROR_THRESHOLD) {
                        LOG_W(LOG_MODULE_FLIGHT, "GPS NO FIX after %d attempts", 
                              SENSOR_ERROR_THRESHOLD);
        ...
        // Vérifier timeout GPS (plus long: 30s sans données)
        if (millis() - g_flight_data.sensors_health.last_gps_success > (SENSOR_TIMEOUT_MS * 6)) {
            if (g_flight_data.sensors_health.gps_healthy) {
                LOG_E(LOG_MODULE_FLIGHT, "GPS TIMEOUT (no data for %d ms)", 
                      millis() - g_flight_data.sensors_health.last_gps_success);
                g_flight_data.sensors_health.gps_healthy = false;
            }
        }
    }
...
float task_flight_get_vertical_acceleration() {
  ...
  float ax_earth = 2 * (qx * qz - qw * qy) * accel_x + 2 * (qy * qz + qw * qx) * accel_y + (qw * qw - qx * qx - qy * qy + qz * qz) * accel_z;
