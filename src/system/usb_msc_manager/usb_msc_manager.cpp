...
// État du gestionnaire USB MSC
static bool msc_initialized = false;

// ============================================================================
// CALLBACKS USB MSC (appelés par la stack USB)
// ============================================================================
// Callbacks USB MSC
...
/**
 * @brief Callback lecture secteur SD via USB
 * 
 * Appelé par la stack USB quand le PC lit des données.
 * Effectue la lecture par secteurs de 512 bytes.
 * 
 * @param[in] lba Logical Block Address (secteur de départ)
 * @param[in] offset Offset dans le secteur
 * @param[out] buffer Buffer où écrire les données
 * @param[in] bufsize Taille à lire
...
/**
 * @brief Callback écriture secteur SD via USB
 * 
 * Appelé par la stack USB quand le PC écrit des données.
 * Effectue l'écriture par secteurs de 512 bytes.
 * 
 * @param[in] lba Logical Block Address (secteur de départ)
 * @param[in] offset Offset dans le secteur
 * @param[in] buffer Buffer contenant les données à écrire
 * @param[in] bufsize Taille à écrire
...
/**
 * @brief Callback start/stop USB (événements power)
 * 
 * Appelé lors d'événements power management du PC.
 * Peut être utilisé pour éjection propre.
 * 
 * @param[in] power_condition Condition power
 * @param[in] start true=démarrage, false=arrêt
 * @param[in] load_eject true=éjection demandée
 * @return true pour accepter l'opération
 */
static bool onStartStop(uint8_t power_condition, bool start, bool load_eject) {
