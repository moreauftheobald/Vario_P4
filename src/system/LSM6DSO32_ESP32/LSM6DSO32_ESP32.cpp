...
// Facteurs de conversion bruts → unités SI (source: datasheet LSM6DSO32)
...
static bool i2c_write_reg(lsm6dso32_device_t* dev, uint8_t reg, uint8_t value) {
  if (!i2c_lock(dev->bus, I2C_TIMEOUT_MS)) {
    LOG_W(LOG_MODULE_IMU, "I2C lock timeout (write)");
    return false;
  }
...
static bool i2c_read_regs(lsm6dso32_device_t* dev, uint8_t reg, uint8_t* data, size_t len) {
  if (!i2c_lock(dev->bus, I2C_TIMEOUT_MS)) {
    LOG_W(LOG_MODULE_IMU, "I2C lock timeout (read)");
    return false;
  }
