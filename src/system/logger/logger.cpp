...
bool logger_init() {
  ...
  Serial.printf("[LOGGER][INFO] Logger initialized, output: %s\n",
                log_output == LOG_OUTPUT_UART ? "UART" : log_output == LOG_OUTPUT_FILE ? "File"
                                                       : log_output == LOG_OUTPUT_BOTH ? "Both"
                                                                                       : "None");
...
/**
 * @brief Fonction principale de logging (version optimisée)
 * 
 * Utilise un buffer statique au lieu d'allouer sur la stack.
 * Économise 320 bytes par appel.
 */
void logger_print(log_module_t module, log_level_t level, const char* format, ...) {
  ...
  // ✅ OPTIMISATION : Prendre le mutex AVANT toute allocation
  if (logger_mutex != NULL) {
  ...
  // ✅ Buffer message temporaire sur stack (256 bytes max)
  char message[256];
  va_list args;
  ...
  // ✅ Formatter dans le BUFFER STATIQUE (protégé par mutex)
