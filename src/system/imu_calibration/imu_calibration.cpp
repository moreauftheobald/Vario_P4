...
void imu_calibration_save_to_config() {
  ...
  // Corriger location : "New" → "LittleFS"
  if (strcmp(g_imu_cal.metadata.location, "New") == 0) {
    strncpy(g_config.imu_calibration.metadata.location, "LittleFS", 16);
    strncpy(g_imu_cal.metadata.location, "LittleFS", 16);
  } else {
...
bool imu_is_stationary() {
  ...
  for (int i = 0; i < IMU_STATIONARY_SAMPLES; i++) {
    ...
    float accel_magnitude = sqrt(accel.acceleration.x * accel.acceleration.x + accel.acceleration.y * accel.acceleration.y + accel.acceleration.z * accel.acceleration.z);
  ...
  bool is_still = (gyro_variance < IMU_STATIONARY_GYRO_THRESHOLD) && (accel_variance < IMU_STATIONARY_ACCEL_THRESHOLD);
...
bool imu_calibration_full() {
  ...
  if (strlen(g_imu_cal.metadata.location) == 0 || strcmp(g_imu_cal.metadata.location, "None") == 0) {
...
int imu_calibration_check_gravity() {
  ...
  // Toujours afficher (WARNING pour voir dans tous les cas)
  LOG_W(LOG_MODULE_IMU, "Gravity: %.3f m/s² (expected: %.3f, error: %.3f)",
        measured_gravity, GRAVITY_STANDARD, error);
...
/**
 * @brief Gère calibration au démarrage
 */
bool imu_calibration_startup() {
  LOG_I(LOG_MODULE_IMU, "Starting IMU calibration process...");

  // Charger calibration depuis config
  if (imu_calibration_load_from_config()) {
    LOG_I(LOG_MODULE_IMU, "Calibration loaded from config");
    imu_calibration_print();

    // Valider qualité
    int cal_score = imu_calibration_validate();

    if (cal_score >= IMU_CAL_SCORE_EXCELLENT) {
      LOG_I(LOG_MODULE_IMU, "Calibration excellent - ready to fly!");
      return true;

    } else if (cal_score >= IMU_CAL_SCORE_ACCEPTABLE) {
      LOG_W(LOG_MODULE_IMU, "Calibration acceptable");

      // Proposer recalibration rapide si immobile
      if (imu_is_stationary()) {
        LOG_I(LOG_MODULE_IMU, "IMU stationary - refining calibration...");
        delay(500);

        if (imu_calibration_quick_gyro()) {
          // Recalculer quality_score
          imu_calibration_validate();

          // Sauvegarder
          imu_calibration_save_to_config();
          config_save();
          config_save_to_littlefs();

          LOG_I(LOG_MODULE_IMU, "Calibration refined and saved");
          imu_calibration_print();
        }
      } else {
        LOG_W(LOG_MODULE_IMU, "IMU moving - using existing calibration");
      }

      return true;

    } else {
      // Calibration mauvaise - forcer recalibration
      LOG_E(LOG_MODULE_IMU, "Calibration INVALID!");
      LOG_E(LOG_MODULE_IMU, "RECALIBRATION REQUIRED");

      LOG_I(LOG_MODULE_IMU, "CALIBRATION IMU NECESSAIRE");
      LOG_I(LOG_MODULE_IMU, "Poser le variometre a plat");
      LOG_I(LOG_MODULE_IMU, "et garder immobile pendant 5s");

      // Attendre immobilité
      LOG_I(LOG_MODULE_IMU, "Waiting for IMU to be stationary...");
      while (!imu_is_stationary()) {
        delay(100);
      }
      ...
      if (imu_calibration_full()) {
        // Valider
        cal_score = imu_calibration_validate();

        if (cal_score >= IMU_CAL_SCORE_ACCEPTABLE) {
          // Sauvegarder
          imu_calibration_save_to_config();
          config_save();
          config_save_to_littlefs();

          LOG_I(LOG_MODULE_IMU, "Calibration complete and saved!");
          imu_calibration_print();

          LOG_I(LOG_MODULE_IMU, "CALIBRATION REUSSIE !");


          delay(2000);
          return true;
        } else {
          LOG_E(LOG_MODULE_IMU, "Calibration still invalid after full calibration!");
          return false;
        }
      } else {
        LOG_E(LOG_MODULE_IMU, "Full calibration failed!");
        return false;
      }
    }
  } else {
    // Pas de calibration - première utilisation
    LOG_W(LOG_MODULE_IMU, "No calibration found - first use");

    LOG_I(LOG_MODULE_IMU, "PREMIERE UTILISATION");
    LOG_I(LOG_MODULE_IMU, "Calibration IMU necessaire");
    LOG_I(LOG_MODULE_IMU, "Poser a plat et garder immobile 5s");
    ...
    LOG_I(LOG_MODULE_IMU, "Starting initial calibration...");
    delay(1000);
    ...
    if (imu_calibration_full()) {
      ...
      LOG_I(LOG_MODULE_IMU, "Initial calibration complete!");
      imu_calibration_print();

      LOG_I(LOG_MODULE_IMU, "CALIBRATION INITIALE REUSSIE !");

      delay(2000);
      return true;
    } else {
      LOG_E(LOG_MODULE_IMU, "Initial calibration failed!");
      return false;
    }
  }
}